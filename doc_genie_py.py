...
Original file is located at
    https://colab.research.google.com/drive/1L2Km1YVrduURPDzwganUdGRKgrUOov6U
"""
...
class DocGenieAnalyzer:
    ...
    @staticmethod
    def analyze_function_logic(func_def):
        analysis = {
            "has_loops": False,
        ...
        desc = f"Executes {func_def.name} operation"
    ...
    def generate_google_docstring(signature, analysis):
        ...
        doc += f"\n    Returns:\n        {signature['return_type']}: Computed result.\n"
        doc += '    """\n'
...
def generate_docstring(code, style):
    ...
    if not signature:
        return None, "❌ Invalid Python function"

    analysis = analyzer.analyze_function_logic(func_def)
    ...
    return final, "✅ Docstring generated"
...
def download_txt():
    ...
    if not generation_history:
        return "❌ Nothing generated", None

    text = "DocGenie Report\n\n"

    for g in generation_history:
        text += g["code"] + "\n"
        text += g["docstring"] + "\n\n"

    name = f"docgenie_{datetime.now().strftime('%H%M%S')}.txt"

    Path(name).write_text(text)

    return "✅ TXT created", name
...
def download_pdf():
    ...
    name = f"docgenie_{datetime.now().strftime('%H%M%S')}.pdf"
    ...
    for g in generation_history:
        ...
        story.append(Paragraph("Original Code", styles["Heading3"]))
        story.append(Preformatted(g["code"], styles["Code"]))

        story.append(Paragraph("Docstring", styles["Heading3"]))
        story.append(Preformatted(g["docstring"], styles["Code"]))
...
def generate_whatsapp_link():
    ...
    msg = "DocGenie generated docstrings!"
...
with gr.Blocks(title="Doc Genie") as demo:
    ...
    style = gr.Radio(["google"], value="google")
    ...
    share_btn = gr.Button("WhatsApp")

    share_out = gr.Textbox()

    share_btn.click(generate_whatsapp_link, outputs=share_out)
